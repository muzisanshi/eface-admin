<template>
  <a-card :bordered="false">
    ...
    <div>
      ...
      <a-pagination class="ant-table-pagination ant-pagination" showSizeChanger :showTotal="total => `总共 ${total} 条`" :pageSize.sync="queryParam.page.pageSize"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      expand(id) {
        this.$api.menu.getPage({pid: id, page: {pageNumber: 1, pageSize: 100}})
          .then(res => {
            for (let i = 0, j = res.records.length; i < j; i++) {
              res.records[i].children = []
              res.records[i].iconName = res.records[i].meta.icon
