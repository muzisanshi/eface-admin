    ...
    <a-spin :spinning="confirmLoading">
      ...
      <a-tabs hideAdd @change="callback" :activeKey="activeKey" type="editable-card" @edit="onEdit">
        ...
        <a-tab-pane tab="识别主机" ref="key2" key="2" :closable="false" class="init-sty">
          <a-form :form="form1">
            <a-input
              v-decorator="['mainEngine.id',{initialValue: this.formData.mainEngine.id}]"
              v-show="false"/>
            <a-card>
              <a-row :gutter="24">
                <a-col :span="8">
              ...
              <a-row :gutter="24">
                <a-col :span="8">
                  <a-input
                    v-decorator="['mainEngine.network.id',{initialValue: this.formData.mainEngine.network?this.formData.mainEngine.network.id:''}]"
                    v-show="false"/>
                  <a-form-item
                    label="IP地址"
                    :labelCol="labelCol"
            ...
            <a-card>
              <a-row :gutter="24">
                <a-col :span="8">
                  ...
                  >
                    <a-input
                      v-decorator="['id',{initialValue: pane.content.id?pane.content.id:''}]"
                      v-show="false"/>
                    <a-select
                      showSearch
                      allowClear
                ...
                <a-col :span="8">
                  <a-input
                    v-decorator="['locationId',{initialValue: pane.content.locationId}]"
                    v-show="false"/>
                  <a-form-item
                    label="设备位置"
                    :labelCol="labelCol"
              ...
              <a-row :gutter="24">
                <a-col :span="8">
                  <a-input
                    v-decorator="['network.id',{initialValue: pane.content.network.id}]"
                    v-show="false"/>
                  <a-form-item
                    label="IP地址"
                    :labelCol="labelCol"
            ...
            <a-card>
              <a-row :gutter="24">
                <a-col :span="8">
                  ...
                  >
                    <a-input
                      v-decorator="['gateBrake.id',{initialValue: pane.content.gateBrake.id}]"
                      v-show="false"/>
                    <a-select
                      showSearch
                      allowClear
              ...
              <a-row :gutter="24">
                <a-col :span="8">
                  <a-input
                    v-decorator="['gateBrake.network.id',{initialValue: pane.content.gateBrake.network.id}]"
                    v-show="false"/>
                  <a-form-item
                    label="IP地址"
                    :labelCol="labelCol"
        ...
        <a-tab-pane
          ...
          v-if="isTemCheck"
          ref="key3"
          :closable="this.thermograph.id ? false : true"
          :key="(panes.length+3) + ''"
          class="init-sty">
          <a-form :form="formTem">
            <a-input
...
export default {
  ...
  methods: {
    ...
    handleSubmit () {
      ...
      validateFields((errors, values) => {
        if (!errors) {
          ...
          const params = Object.assign(this.formData, values)

          const camerasData = []; let isSubmit = true
    ...
    remove(targetKey) {
      const that = this
      const allTabsNum = that.panes.length + 3
