...
<template>
  ...
  >
    <a-spin :spinning="confirmLoading">
      <a-tabs @change="callback" :activeKey="activeKey" type="card" @edit="onEdit">
            ...
            <a-card class="init-sty">
              <!-- <a-row :gutter="24">
                <a-col :span="12" class="upload-title">
                  <a-form-item label="头像" :labelCol="labelCol" :wrapperCol="wrapperCol">
                    <div
                      style="width: 60px;height: 60px;border: 1px solid #cccccc;overflow: hidden;display: inline-block;border-radius: 50%"
                    >
              ...
              <a-row :gutter="24">
                <!-- <a-col :span="12" v-if="userType === 'OWNER'">
                  <a-form-item label="关系" :labelCol="labelCol" :wrapperCol="wrapperCol">
                    <a-select
                      showSearch
                      placeholder="选择关系"
                ...
                <a-col :span="12">
                  ...
                  >
                    <a-input
                      :maxLength="32"
                      v-decorator="['phoneNo', {initialValue: this.formData.phoneNo, rules: [{required: true, message: '请输入电话号码！'}]}]"
                    >
                      <a-select
                        showSearch
                        slot="addonBefore"
                ...
                <a-col :span="12">
                  <a-form-item label="性別" :labelCol="labelCol" :wrapperCol="wrapperCol">
                    <a-select
                      ...
                      v-decorator="['sexual', {initialValue: this.formData.sexual,rules: [{required: true, message: '请选择性別！'}]}]"
                    ></a-select>
                  </a-form-item>
                </a-col>
                ...
                <a-col :span="12" v-if="userType === 'VISITOR'">
                  <a-input
                    v-decorator="['visitorExtendInfo.id',{initialValue: this.formData.visitorExtendInfo?this.formData.visitorExtendInfo.id:''}]"
                    v-show="false"
                  />
                  <a-input
                    v-decorator="['visitorExtendInfo.interviewerId',{initialValue: this.formData.visitorExtendInfo?this.formData.visitorExtendInfo.interviewerId:''}]"
                    v-show="false"
                  />
                  <a-form-item
                    label="受访用户"
                  ...
                  >
                    <a-input
                      @click="selectUser()"
                      :read-only="true"
                      v-decorator="['visitorExtendInfo.interviewerRealName', {initialValue: this.formData.visitorExtendInfo?this.formData.visitorExtendInfo.interviewerRealName:'', rules: [{required: true, message: '请选择受访用户！'}]}]"
                    />
                  </a-form-item>
                </a-col>
                ...
                <a-col :span="12">
                  <a-form-item label="备注" :labelCol="labelCol" :wrapperCol="wrapperCol">
                    <a-input
                      :maxLength="64"
                      v-decorator="['remark', {initialValue: this.formData.remark, rules: [{required: false, message: '请输入备注！'}]}]"
                    />
                  </a-form-item>
                </a-col>
              </a-row>
          ...
          <a-form :form="pane.form">
            <a-card class="init-sty">
              <a-row :gutter="24">
                ...
                <!-- <a-col :span="8">
                  <a-form-item label="是否解绑" :labelCol="labelCol" :wrapperCol="wrapperCol">
                    <a-switch
                      :checked="pane.content.relieve"
                      @change="changeRelieve($event,index)"
                      v-decorator="['pane.content.relieve']"
                    />
                  </a-form-item>
                </a-col> -->
              </a-row>
                    ...
                    <a-row :gutter="24">
                      ...
                      <a-col :span="6">
                        <a-form-item v-bind="formLayout" label="是否启用" :required="false">
                          <a-switch
                            :checked="pane.content.gateBrakeLimits[index1].enable"
                            @change="changeGkEnable($event,index,index1)"
                            v-decorator="['pane.content.gateBrakeLimits[index1].enable']"
                          />
                        </a-form-item>
                      </a-col>
                    </a-row>
                    ...
                    <a-row :gutter="24" class="init-ant-form-item">
                      <a-col :span="9">
                        <a-form-item v-bind="formLayout" label="开始时间" :required="true">
                          <a-date-picker
                            @change="(value, dateString) => beginDateChange(value,dateString, index, index1)"
                            v-if="pane.content.gateBrakeLimits[index1].beginDatetime ? true :false"
                            :value="moment(pane.content.gateBrakeLimits[index1].beginDatetime,'YYYY-MM-DD HH:mm:ss')"
                            format="YYYY-MM-DD HH:mm:ss"
                            showTime
                          />
                          <a-date-picker
                            class="clearStartDate"
                            @change="(value, dateString) => beginDateChange(value,dateString, index, index1)"
                            v-if="pane.content.gateBrakeLimits[index1].beginDatetime ? false :true"
                            format="YYYY-MM-DD HH:mm:ss"
                            showTime
                          />
                        </a-form-item>
                      ...
                      <a-col :span="9">
                        <a-form-item v-bind="formLayout" label="结束时间" :required="false">
                          <a-date-picker
                            @change="(value, dateString) => endDateChange(value,dateString, index, index1)"
                            v-if="pane.content.gateBrakeLimits[index1].endDatetime"
                            :value="moment(pane.content.gateBrakeLimits[index1].endDatetime,'YYYY-MM-DD HH:mm:ss')"
                            format="YYYY-MM-DD HH:mm:ss"
                            showTime
                          />
                          <a-date-picker
                        ...
                        <div
                          style="margin-bottom: 12px;color: #9a9a9a;font-size: 12px;text-align: right;"
                        >
                          <span>如果不选择结束时间，则按照门禁闸机常用规则进行限制</span>
                        </div>
                      </a-col>
                      ...
                      <a-col :span="4" :offset="2" v-if="!pane.content.gateBrakeLimits[index1].id">
                        <a-icon
                          ...
                          type="minus-circle-o"
                          @click="() => removeAttr(k,index,index1)"
                        />
                      </a-col>
                    </a-row>
                  </div>
    ...
    <!-- <select-room
      ref="selectRoom"
      :estateId="panes[curPaneIndex].content.estateId"
      @selectRoom="selectRoomSuccess"
    ></select-room> -->
...
export default {
  ...
  methods: {
    ...
    add() {
      ...
      validateFields((errors, values) => {
        if (!errors) {
          ...
          this.newTabIndex =
            parseInt(
              this.panes[this.panes.length - 1].title.split('域')[1]
                ? this.panes[this.panes.length - 1].title.split('域')[1]
                : 0
            ) + 3
    ...
    removeAttr(k, index, index1) {
      ...
      this.panes[index].content.gateBrakeLimits = this.panes[index].content.gateBrakeLimits.filter(
        (key, inx) => inx !== index1
      )
    ...
    handleSubmit() {
      ...
      if (isSubmit) {
        ...
        if (this.title === '新增') {
          // this.formData.estateIds = camerasData[0].estateId.split(',')
          this.$api.userDanger
            .save(this.formData)
            .then(res => {
              this.$notification.success({
...
.select-init form .ant-input-group .ant-select,
